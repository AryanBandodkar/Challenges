...
> Note: This is the original reference documentation baseline.  
> For this repository's current implementation status and intentional differences, see `docs/original-system-alignment.md` and `README.md`.
...
11. [Creating a Backend Variant (Express / NestJS)](#creating-a-backend-variant-express--nestjs)
...
- **Multiple courses** (currently 3: React Fundamentals, Redux & RTK Query, Next.js App Router)
- **Multiple challenges per course** (e.g. 23, 13, 17 per course in this repo; scalable to 100+)
...
### 4. GitHub Actions Automation (Runs on Push / Commit)
...
1. **validate** job  
   Checkout (fetch-depth: 2), setup Node, run `node scripts/ci-validate.js`.

2. **review** job (after validate)  
   - Checkout, setup Node, cache npm.  
   - Install dependencies per course (`project/` and `review-engine/`), install Playwright browsers.  
   - **Get changed files**:  
     - On **push**: `git diff --name-only HEAD~1 HEAD`  
     - On **pull_request**: `git diff --name-only <base-sha> HEAD`  
   - **If there are changed files**: run `run-review-changed.js --ref HEAD~1` (or base SHA for PR) so only affected challenges are reviewed.  
   - **If no changed files** (e.g. empty diff): run `global-review/run-all-reviews.js` (full review).  
   - Run `update-progress.js` to refresh `learner-results/progress.json` and README evidence.  
   - **Commit and push**: stage `courses/*/results/*.json`, `pathway-review/*.json`, `learner-results/progress.json`, course and root READMEs; commit with message `chore: update review results and README evidence [skip ci]`; push (so the repo gets updated results after each run). `continue-on-error: true` so the job doesn’t fail if push is rejected (e.g. no write permission).  
   - Upload review results as an artifact (retention 30 days).

**Secrets**: `GROQ_API_KEY` (optional) for AI review; without it, AI score is 0 but other layers still run.

---

### 5. Learner Workflow (Origin vs Upstream)

This repo is designed so **learners** work in their own copy and keep the **course repo** as a read-only source of updates.

**Remotes**:
- **origin** → learner’s own GitHub repo (where they push their challenge work).
- **upstream** → course repo (e.g. `https://github.com/sparkplustech/challenge-engine-react.git`). Read-only; used only to pull latest challenges and course updates.

**Typical learner flow**:
1. Clone the course repo (or their fork).  
2. Set **origin** to their own repo: `git remote set-url origin https://github.com/<username>/<their-repo>.git`  
3. Add **upstream** if missing: `git remote add upstream https://github.com/sparkplustech/challenge-engine-react.git`  
4. Run `npm run setup` (one-time).  
5. Work on challenges in `courses/<course>/project/`; run reviews from dashboard or CLI (`run-review-challenge`, `run-review-course`).  
6. **Push work**: `git push origin main` (or their branch). Pushing to **origin** triggers the GitHub Actions workflow **on the learner’s repo** (if they use this workflow): review runs, results are committed and pushed back to their repo.  
7. **Get course updates**: `git fetch upstream`, `git merge upstream/main` (or pull from upstream). So they keep their work on origin and pull only course/content changes from upstream.

**Important**: The workflow that runs “on push” runs in **the repo that was pushed to**. So if a learner pushes to **their** origin, the workflow runs on their repo (and can commit results there if they have Actions enabled). If they push to the main course repo (e.g. upstream), the workflow runs there. Learners should push to **origin** (their repo), not upstream (course repo), unless they are maintainers.

**Summary for backend variant**: Same idea—document that learners set origin to their repo, upstream to the “course” or template repo; push to origin; pull from upstream for updates; and that the workflow runs on push/PR and optionally commits results back.
...
**Calculation**:
- `averageScore`: Weighted average of all challenge scores (only challenges with a result; empty courses can show 0)
- `completionPercentage`: `(completedChallenges / totalChallenges) * 100` (completed = passed ≥ minScore)
...
**Course Weights** (from `pathway-review/pathway-config.json`):
- React Fundamentals: 0.33
- Redux & RTK Query: 0.33
...
## Creating a Backend Variant (Express / NestJS)

This section is the **single reference** for creating a new challenge-engine repo that follows the same structure, process, and methods but for **backend** stacks (e.g. Express, NestJS). Use it as a checklist and blueprint.

### What Stays the Same (Reuse As-Is or Copy)

| Area | What to keep |
|------|----------------|
| **Repo layout** | `courses/`, `pathway-review/`, `scripts/`, `dashboard/`, `learner-results/`, `global-review/` |
| **Config-driven design** | Courses from `pathway-config.json`; challenges from each `course-config.json`; no code changes needed to add courses/challenges |
| **Challenge definition** | Each challenge = folder under `project/challenges/{id}/` with `README.md` (instructions + Technical Requirements) and `metadata.json` |
| **metadata.json** | Must include `challengeId`, `challengeName`, `filesToCheck` (project-relative paths), `skills`, and optionally `patternsRequired` for architecture checks |
| **Smart review** | `file-to-challenge-map.js` builds map from pathway config + each challenge’s `metadata.filesToCheck`; same script pattern for “review changed only” |
| **Results & progress** | Same flow: `challenge-results.json` → `course-summary.json` → `progress.json`; merge-by-challenge when running single-challenge review |
| **README evidence** | Same: `update-readme-evidence.js` updates course project README and root README with completion %, score, pass/fail |
| **Dashboard concept** | Same API shape: `/api/progress`, `/api/courses`, `/api/courses/:id/challenges`, `/api/review` (POST); UI can stay or be restyled |
| **Scripts** | Same entrypoints: `setup.js`, `update-progress.js`, `update-readme-evidence.js`, `run-review-challenge.js`, `run-review-course.js`, `get-changed-challenges.js`, `run-review-changed.js`; global `run-all-reviews.js` |
| **Pathway** | Same: `pathway-config.json` (course list, weights, badge levels); pathway summary and scoring from `global-review/` |

### What Changes for Backend (Express / NestJS)

| Area | Frontend (this repo) | Backend variant |
|------|----------------------|------------------|
| **Project type** | Vite + React app in `project/` | Node app (Express or NestJS) in `project/` |
| **Functional tests** | Vitest + React Testing Library; `project/tests/challenge-*.test.tsx` | Jest or Vitest for Node; `project/tests/challenge-*.test.js` (or `.ts`); test routes/handlers/services, not components |
| **E2E tests** | Playwright against browser (DOM) | Supertest (or similar) against running app, or Playwright for API; `project/tests/e2e/challenge-*.spec.js` |
| **Test runner in review** | `review-engine/test-runner.js` runs Vitest | Adapt to run Jest/Vitest for Node (same interface: run suite, parse JSON reporter, return score) |
| **E2E runner** | `review-engine/e2e-runner.js` runs Playwright | Adapt to run Supertest/API E2E (start app, hit endpoints, assert status/body) |
| **Linter** | ESLint for TS/React in `project/` | ESLint for Node (same `linter.js` interface: run ESLint, map errors to score) |
| **Architecture checker** | AST checks for React patterns (functionalComponent, useState, etc.) | AST checks for backend patterns (e.g. Express: `app.get/post`, `router.use`; NestJS: `@Controller`, `@Get`, services, modules) |
| **Best practices** | Heuristics for React (file structure, naming) | Heuristics for Node (async/await, error handling, route structure) |
| **AI review** | Same: send README + learner code; Groq (or other) returns score + feedback | Same; prompt can ask for backend-specific criteria (security, status codes, validation) |
| **Scoring weights** | e.g. functional 35%, codeQuality 15%, architecture 10%, bestPractices 10%, e2e 15%, ai 15% | Same six layers; weights can be tuned (e.g. higher E2E for API coverage) |
| **Stubs** | Components that `return null` or minimal JSX with required `id`s | Route handlers that return 501 or minimal response; services that throw “not implemented” until learner implements |

### Backend Blueprint: Step-by-Step

1. **New repo**  
   Clone or copy this repo; strip course-specific content but keep: `scripts/`, `dashboard/`, `global-review/`, `pathway-review/` (configs), `learner-results/`.

2. **Pathway config**  
   `pathway-review/pathway-config.json`: define courses, e.g. `04-express-basics`, `05-nestjs-basics` (or one course per stack). Set weights and badge levels.

3. **Per-course layout**  
   For each course (e.g. `courses/04-express-basics/`):
   - **project/**  
     Express or NestJS app (learner code here). Same idea: stubs only; learner implements what each challenge README asks.
   - **review-engine/**  
     Copy from `01-react-fundamentals/review-engine/`, then replace:
     - **test-runner.js**: run Jest/Vitest for Node; parse standard JSON reporter; return `{ score, passed, totalTests, passedTests, ... }`.
     - **e2e-runner.js**: start app (or use in-process), run API E2E (Supertest/Playwright); return same shape.
     - **linter.js**: run ESLint in `project/` (config for Node); same scoring formula.
     - **architecture-checker.js**: use `@babel/parser` + traverse to require patterns (e.g. `express.Router`, `app.use`, or Nest `@Controller`, `@Injectable`). Read `metadata.patternsRequired` per challenge.
     - **best-practices.js**: adapt heuristics to Node (error handling, async, no sync I/O in request path, etc.).
   - **ai-review/index.js**  
     Same pattern: read challenge README + `filesToCheck` contents; call Groq (or other); return score + strengths/improvements.
   - **course-config.json**  
     Same: `challenges` array, `scoring` weights (sum 1.0), `requirements.minScore` / `minCompletion`.

4. **Challenges**  
   Under `project/challenges/{challenge-id}/`:
   - **README.md**  
     Goal, instructions, **Technical Requirements** (what tests/architecture expect).
   - **metadata.json**  
     `challengeId`, `challengeName`, `filesToCheck` (e.g. `["src/routes/tasks.js"]`), `skills`, `patternsRequired` (for architecture checker).

5. **Tests**  
   - Unit/integration: `project/tests/challenge-{id}.test.js` (or `.ts`) — import app or handlers, assert behavior.
   - E2E: `project/tests/e2e/challenge-{id}.spec.js` — HTTP requests, status codes, response body.

6. **Scripts**  
   Keep same CLI: `run-review-challenge.js --course=04-express-basics --challenge=01-hello-route`; review engine path is `courses/{course}/review-engine/index.js`. No script changes if you follow the same course/challenge layout and config.

7. **Dashboard**  
   Same server API; point it at the new repo root. Progress and course/challenge lists come from `pathway-config.json` and `course-config.json`; challenge detail from `challenges/{id}/README.md` and `metadata.json`.

8. **Learner handoff**  
   Same as this repo: empty `challenge-results.json`, zero completion in `course-summary.json`, run `update-progress.js` so `progress.json` and README evidence show 0%.

### One-Page Checklist for a New Backend Engine

- [ ] Pathway: `pathway-config.json` has all courses (Express/NestJS).
- [ ] Each course: `course-config.json` with challenges; `project/` is runnable Node app (stubs only).
- [ ] Each challenge: `README.md` + `metadata.json` with `filesToCheck` and optional `patternsRequired`.
- [ ] Tests: `project/tests/challenge-*.test.js` and `project/tests/e2e/challenge-*.spec.js`; runner in review engine runs them and parses output.
- [ ] Review engine: test-runner (Jest/Vitest Node), e2e-runner (Supertest/API), linter, architecture-checker (AST for routes/controllers), best-practices, ai-review; merge results by challenge; write `challenge-results.json` → course summary → `update-progress.js` → `update-readme-evidence.js`.
- [ ] Scripts: `run-review-challenge`, `run-review-course`, `run-review-changed` (using same file-to-challenge map from `metadata.filesToCheck`).
- [ ] Dashboard: same endpoints; progress from `learner-results/progress.json`.
- [ ] Learner-ready: no solution code; stubs so app runs and tests can target routes/handlers.
